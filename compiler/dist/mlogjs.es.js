...
const arithmeticBinaryOperators = [
  "*",
  "**",
  "+",
  "-",
...
const comparisonBinaryOperators = [
  "!=",
  "!==",
  "<",
  "<=",
...
const singleOperators = [...unaryOperators, ...updateOperators];
const operators = [...leftRightOperators, ...singleOperators];
const operatorMap$2 = {
  "==": "equal",
  "===": "strictEqual",
  "!=": "notEqual",
...
class OperationInstruction extends InstructionBase {
  constructor(kind, temp, left, right) {
    super("op", kind, temp, left, right);
  }
}
class SetInstruction extends InstructionBase {
  constructor(store, value) {
    super("set", store, value);
  }
}
class EndInstruction extends InstructionBase {
  constructor() {
    super("end");
  }
}
class AddressResolver extends InstructionBase {
  get hidden() {
    return true;
  }
  set hidden(value) {
  ...
  resolve(i) {
    for (const literal2 of this.bonds)
      literal2.data = i;
  }
  bind(bond) {
...
var EJumpKind = /* @__PURE__ */ ((EJumpKind2) => {
  EJumpKind2["Equal"] = "equal";
  EJumpKind2["NotEqual"] = "notEqual";
  EJumpKind2["LessThan"] = "lessThan";
  EJumpKind2["LessThanEq"] = "lessThanEq";
  EJumpKind2["GreaterThan"] = "greaterThan";
  EJumpKind2["GreaterThanEq"] = "greaterThanEq";
  EJumpKind2["StrictEqual"] = "strictEqual";
  EJumpKind2["Always"] = "always";
  return EJumpKind2;
})(EJumpKind || {});
class JumpInstruction extends InstructionBase {
  constructor(address, kind, left = null, right = null) {
    super("jump", address, kind, left, right);
  }
}
class SetCounterInstruction extends InstructionBase {
  constructor(value) {
    super("set", "@counter", value);
  }
}
class ZeroSpaceInstruction extends InstructionBase {
  toString() {
    return this.args.join("");
  }
}
class BreakInstruction extends JumpInstruction {
  constructor(address) {
    super(address, EJumpKind.Always);
  }
}
class ContinueInstruction extends JumpInstruction {
  constructor(address) {
    super(address, EJumpKind.Always);
  }
}
...
class BaseValue extends VoidValue {
  "u-"(scope) {
...
for (const key in operatorMap$1) {
  const kind = operatorMap$1[key];
...
const unaryOperatorMap$1 = {
  "!": "not",
  "~": "flip"
};
for (const key in unaryOperatorMap$1) {
  const name = unaryOperatorMap$1[key];
...
class StoreValue extends BaseValue {
  ...
  "="(scope, value) {
    ...
    if (value instanceof TempValue && value.canProxy)
...
class TempValue extends StoreValue {
  constructor(scope, name) {
    super(scope, name != null ? name : "t" + scope.ntemp + (scope.name ? ":" + scope.name : ""));
    this.canProxy = true;
    if (!name)
      scope.ntemp++;
  }
...
class ObjectValue extends VoidValue {
  constructor(scope, data2 = {}) {
    super(scope);
    this.constant = true;
    this.macro = true;
...
for (const op of operators) {
  ObjectValue.prototype[op] = function(...args) {
    const $ = this.data[`$${op}`];
    if (!$)
      return VoidValue.prototype[op].apply(this, args);
...
class Scope2 {
  ...
  copy() {
    const scope = new Scope2(__spreadValues({}, this.data), this.parent, this.stacked, this.ntemp, this.name, this.inst);
  ...
  createFunction(name, stacked) {
    return new Scope2({}, this, stacked != null ? stacked : this.stacked, 0, name, this.inst);
  }
  has(name) {
...
const Program = (c, scope, node) => {
  return c.handleMany(scope != null ? scope : new Scope2({}), node.body);
};
...
const VariableDeclaration = (c, scope, node) => {
  return c.handleMany(scope, node.declarations, (scope2, child) => VariableDeclarator(c, scope2, child, node.kind));
};
const VariableDeclarator = (c, scope, node, kind = "let") => {
  ...
  switch (node.id.type) {
    case "Identifier": {
      ...
      if (kind === "const" && (init == null ? void 0 : init.constant)) {
        scope.set(name, init);
        return valinst;
      } else {
...
const BinaryExpression = LRExpression;
const LogicalExpression = LRExpression;
...
const IfStatement = (c, scope, node) => {
  ...
  inst.push(...testInst, new JumpInstruction(endIfAddr, EJumpKind.Equal, test, new LiteralValue(scope, 0)), ...c.handle(scope, node.consequent)[1], new AddressResolver(endIfAddr));
  const endElseAddr = new LiteralValue(scope, null);
  if (node.alternate)
    inst.push(new JumpInstruction(endElseAddr, EJumpKind.Always), new AddressResolver(endIfAddr), ...c.handle(scope, node.alternate)[1], new AddressResolver(endElseAddr));
  return [null, inst];
};
const WhileStatement = (c, scope, node) => {
  ...
  if (test instanceof LiteralValue) {
    if (test.data) {
      lines.push(startLoopLine, ...c.handle(scope, node.body)[1], new JumpInstruction(startLoopAddr, EJumpKind.Always), endLoopLine);
    }
    return [null, lines];
  }
  lines.push(startLoopLine, ...testLines, new JumpInstruction(endLoopAddr, EJumpKind.Equal, test, new LiteralValue(scope, 0)), ...c.handle(scope, node.body)[1], new JumpInstruction(startLoopAddr, EJumpKind.Always), endLoopLine);
  return [null, lines];
};
const ForStatement = (c, scope, node) => {
  scope = scope.createScope();
  const initInst = node.init ? c.handle(scope, node.init)[1] : [];
  const [test, testLines] = node.test ? c.handleEval(scope, node.test) : [new LiteralValue(scope, 1), []];
...
class FunctionValue extends VoidValue {
  ...
  normalCall(scope, args) {
    ...
    const inst = this.paramStores.map((param, i) => param["="](scope, args[i])[1]).reduce((s, c) => s.concat(c), []).concat(...this.ret["="](scope, callAddressLiteral)[1], new JumpInstruction(this.addr, EJumpKind.Always), new AddressResolver(callAddressLiteral));
...
const ArrowFunctionExpression = (c, scope, node) => {
  ...
  return [
    new FunctionValue(scope, name, paramNames, paramStores, body, c),
    []
  ];
};
const FunctionDeclaration = (c, scope, node) => {
  return [
    scope.set(node.id.name, ArrowFunctionExpression(c, scope, node)[0]),
    []
  ];
};
const FunctionExpression = ArrowFunctionExpression;
const CallExpression = (c, scope, node) => {
  const inst = [];
  const args = node.arguments.map((node2) => {
    const [v, i] = c.handleEval(scope, node2);
    inst.push(...i);
    return v;
  });
...
const ObjectExpression = (c, scope, node) => {
  ...
  return [new ObjectValue(scope, data2), inst];
};
...
const MemberExpression = (c, scope, node) => {
  ...
  const [prop, propInst] = node.computed ? c.handleEval(scope, node.property) : [new LiteralValue(scope, node.property.name), []];
...
var handlers = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  Program,
  VariableDeclaration,
  VariableDeclarator,
...
class MacroFunction extends VoidValue {
  ...
  for (const key in mathOperations) {
    ...
    macroMathOperations[key] = new MacroFunction(scope, (a, b) => {
      if (b) {
        ...
        const temp2 = new TempValue(scope);
        return [temp2, [new OperationInstruction(key, temp2, a, b)]];
      }
...
class MlogMath extends ObjectValue {
  constructor(scope) {
    super(scope, createMacroMathOperations(scope));
  }
}
class Concat extends MacroFunction {
  constructor(scope) {
    super(scope, (...values) => {
      ...
      return [
        new LiteralValue(scope, values.map((v) => v.data).join("")),
        []
      ];
    });
  }
}
class Draw extends MacroFunction {
  constructor(scope) {
    super(scope, (kind, ...args) => {
      ...
      return [
        null,
        [
          new InstructionBase("draw", kind.data, ...args.map((v) => {
...
class Print extends MacroFunction {
  constructor(scope) {
    super(scope, (...values) => {
      const inst = [];
      for (const value of values) {
...
class PrintFlush extends MacroFunction {
  constructor(scope) {
    super(scope, (target) => {
...
class DrawFlush extends MacroFunction {
  constructor(scope) {
    super(scope, (target) => {
      if (!(target instanceof ObjectValue))
...
class Building extends ObjectValue {
  toString() {
    return this.name;
  }
  constructor(scope, name) {
    super(scope, {
      $get: new MacroFunction(scope, (prop) => {
        if (prop instanceof LiteralValue && typeof prop.data === "string") {
          const name2 = itemNames.includes(prop.data) ? camelToDashCase(prop.data) : prop.data;
          const temp = new TempValue(scope);
          return [
            temp,
            [new InstructionBase("sensor", temp, this, `@${name2}`)]
          ];
        }
...
class BuildingBuilder extends ObjectValue {
  constructor(scope) {
    super(scope, {
      $call: new MacroFunction(scope, (nameLit) => {
...
for (const key in operatorMap$2) {
  const kind = operatorMap$2[key];
...
class GetLink extends MacroFunction {
  constructor(scope) {
    super(scope, (index) => {
      if (!(index instanceof StoreValue) && (!(index instanceof LiteralValue) || typeof index.data !== "number"))
...
const validKinds$1 = ["enabled", "shoot", "shootp", "config", "color"];
class Control extends MacroFunction {
  constructor(scope) {
    super(scope, (kind, building, ...args) => {
      if (!(kind instanceof LiteralValue) || typeof kind.data !== "string")
        throw new Error("The control kind must be a string literal");
      if (!validKinds$1.includes(kind.data))
...
class NamespaceMacro extends ObjectValue {
  constructor(scope, { changeCasing = false } = {}) {
    super(scope, {
      $get: new MacroFunction(scope, (prop) => {
        if (!(prop instanceof LiteralValue) || typeof prop.data !== "string")
          throw new Error("Cannot use dynamic properties on object macros");
        const symbolName = this.changeCasing ? camelToDashCase(prop.data) : prop.data;
  ...
  constructor(scope) {
    ...
    this.data.$get = new MacroFunction(scope, (prop) => {
      if (prop instanceof LiteralValue) {
        ...
        if (prop.data === "this")
          return [new Building(scope, "@this"), []];
      }
    ...
    super(scope, {
      $get: new MacroFunction(scope, (prop) => {
        if (prop instanceof LiteralValue && typeof prop.data === "string") {
          ...
          return [
            result,
            [new InstructionBase("sensor", result, this, `@${name2}`)]
          ];
        }
        if (prop instanceof StoreValue) {
          const temp = new TempValue(scope);
...
class Radar extends MacroFunction {
  constructor(scope) {
    super(scope, (building, filter1, filter2, filter3, order, sort) => {
      if (!(building instanceof ObjectValue))
        throw new Error("The building must a store");
      if (!(filter1 instanceof LiteralValue) || typeof filter1.data !== "string" || !(filter2 instanceof LiteralValue) || typeof filter2.data !== "string" || !(filter3 instanceof LiteralValue) || typeof filter3.data !== "string")
      ...
      return [
        resultUnit,
        [
          new InstructionBase("radar", filter1.data, filter2.data, filter3.data, sort.data, building, order, temp)
        ]
      ];
    });
  }
}
class Sensor extends MacroFunction {
  constructor(scope) {
...
class Wait extends MacroFunction {
  constructor(scope) {
    super(scope, (seconds) => {
      if (!(seconds instanceof StoreValue) && !(seconds instanceof LiteralValue && typeof seconds.data === "number"))
...
class Lookup extends MacroFunction {
  constructor(scope) {
    super(scope, (kind, index) => {
      ...
      if (!(index instanceof LiteralValue && typeof index.data === "number") && !(index instanceof StoreValue))
...
class UnitBind extends MacroFunction {
  constructor(scope) {
    super(scope, (type) => {
...
class UnitControl extends MacroFunction {
  constructor(scope) {
    super(scope, (mode, ...args) => {
      ...
      if (args.some((value) => !(value instanceof StoreValue || value instanceof LiteralValue || value instanceof ObjectValue)))
...
class UnitRadar extends MacroFunction {
  constructor(scope) {
    super(scope, (filter1, filter2, filter3, order, sort) => {
      ...
      return [
        temp,
        [
          new InstructionBase("uradar", filter1.data, filter2.data, filter3.data, order, sort.data, temp)
        ]
      ];
    });
  }
...
class UnitLocate extends MacroFunction {
  constructor(scope) {
    super(scope, (find, ...args) => {
      ...
      if (args.some((value) => !(value instanceof StoreValue || value instanceof LiteralValue)))
      ...
      switch (find.data) {
        ...
        case "building": {
          ...
          if (!(group instanceof LiteralValue && typeof group.data === "string")) {
...
class MemoryBuilder extends ObjectValue {
  constructor(scope) {
    super(scope, {
      $call: new MacroFunction(scope, (cell, size = new LiteralValue(scope, 64)) => {
...
class RawValueMacro extends MacroFunction {
  constructor(scope) {
    super(scope, (name) => {
      if (!(name instanceof LiteralValue) || typeof name.data !== "string")
...
function initScope(scope) {
  ...
  scope.hardSet("printFlush", new PrintFlush(scope));
  scope.hardSet("drawFlush", new DrawFlush(scope));
  scope.hardSet("getLink", new GetLink(scope));
  scope.hardSet("control", new Control(scope));
  scope.hardSet("radar", new Radar(scope));
  scope.hardSet("sensor", new Sensor(scope));
  scope.hardSet("wait", new Wait(scope));
  scope.hardSet("lookup", new Lookup(scope));
  scope.hardSet("unitBind", new UnitBind(scope));
  scope.hardSet("unitControl", new UnitControl(scope));
  scope.hardSet("unitRadar", new UnitRadar(scope));
  scope.hardSet("unitLocate", new UnitLocate(scope));
}
class Compiler {
  ...
  compile(script) {
    ...
    try {
      ...
      return [null, error, (_a = error == null ? void 0 : error.nodeStack) != null ? _a : []];
    }
    return [output, null, []];
  }
  resolve(valueInst) {
  ...
  handle(scope, node) {
    ...
    try {
      ...
      (_a = error.nodeStack) != null ? _a : error.nodeStack = [];
